# Analysis of black walnut Wright spring data
# VERSION 2
...
#variable of interest: leafOutDate_May
...
cor_spring = bw.clim[names(bw.clim) %in% c("leafOutDate_May", "daylength_diff", "DD_0_wt", "DD_0_sp", 
                                          "DD5_sp", "DD5_wt", "MAT", "MWMT", "MCMT", "TD", "MAP", "bFFP")]
...
summary(model.day)
plot(model.day)
# daylength strong, maybe quad
model.day2 = glm(BudBurst_JD ~ daylength_diff + I(daylength_diff^2), data = bw.clim)
summary(model.day2)
plot(model.day2)
#quadratic term significant
#pretty good start, some repeat vals
rss = sum(model.day2$residuals^2); r2 = 1-rss/tss
...
summary(model.dd0wt)
plot(model.dd0wt)
# lot of repeat vals, quad
model.dd0wt2 = glm(BudBurst_JD ~ DD_0_wt + I(DD_0_wt^2), data = bw.clim)
summary(model.dd0wt2)
plot(model.dd0wt2)
#pretty good but residuals still not great
rss = sum(model.dd0wt2$residuals^2); r2 = 1-rss/tss
p = length(coef(model.dd0wt2))-1
...
plot(model.dd5wt)
# very good
ggplot(data = bw.clim, aes(DD5_wt, BudBurst_JD))+
...
model.mat2 = glm(BudBurst_JD ~ MAT + I(MAT^2), data = bw.clim)
summary(model.mat2)
plot(model.mat2)
#okay again, quad not sig
...
plot(model.mcmt2)
# pretty good
rss = sum(model.mcmt2$residuals^2); r2 = 1-rss/tss
p = length(coef(model.mcmt2))-1
...
#winter temperature
anova(model.null, model.day2, model.dd0wt2, model.dd0sp, model.dd5wt, model.dd5sp, model.mat, model.mcmt2, model.mwmt, model.td, model.map, model.bffp)
aics = AIC(model.null, model.day2, model.dd0wt2, model.dd0sp, model.dd5wt, model.dd5sp, model.mat, model.mcmt2, model.mwmt, model.td, model.map, model.bffp)
...
#save the results
write.csv(results, "BlackWalnutWrightSpringResults.csv")



#run cross validation to assess best model
models = list(model.null, model.day2, model.dd0wt2, model.dd0sp, model.dd5wt, model.dd5sp, model.mat, model.mcmt2, model.mwmt, model.td, model.map, model.bffp)
set.seed(12022025)
cv.error = rep(0, 12)
for (i in 1:length(models)){
  cv.error[i] = cv.glm(bw.clim, models[[i]])$delta[2]
}
# DD5_wt, DD_0_wt or MCMT
ggplot(data = bw.clim, aes(DD_0_wt, BudBurst_JD))+
  geom_point()+geom_smooth(method = 'lm', se=F, formula = y~I(1/x))
summary(model.dd0wt2)
plot(model.dd0wt2)
...
#calculate the r^2
rss = sum(model.mcmt2$residuals^2)
tss = sum((bw.clim$BudBurst_JD - mean(bw.clim$BudBurst_JD))^2)
r2 = 1-rss/tss
n = nrow(bw.clim)
p = length(coef(model.mcmt2))-1
